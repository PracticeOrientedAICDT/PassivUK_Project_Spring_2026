...
def temp_at(data, target_dt, zone):
    dts  = get_dts(data)
    n    = len(dts)
    room = get_state(data, f"room_temp.z{zone}", n)

    idx = max((i for i, dt in enumerate(dts) if dt <= target_dt), default=None)
    return room[idx] if idx is not None else None
...
def stage2_override_payload(override_dt, conditions):
    """
    Override logic:
    - Overridden zones: model sees only the boost setpoint with no future entries,
      so it treats the override as permanent and optimises accordingly.
      Stage 3 recalculates from scratch at OVERRIDE_END_HOUR.
    ...
    def make(sched, apply_override):
        hours, temps = [], []
        if apply_override:
            # Boost zone: single entry at override time, no future schedule visible
            hours.append(override_hour)
            temps.append(OVERRIDE_SETPOINT)
        else:
            # Non-boost zone: only include entries within the override window
            # (>= override_hour and < OVERRIDE_END_HOUR) so the 10:00 frost
            # entry doesn't bleed into the optimiser's decision-making
            for h, c in zip(sched["hours"], sched["C"]):
                if override_hour <= h < OVERRIDE_END_HOUR:
                    hours.append(h)
                    temps.append(c)
            if not hours:
                # No explicit entry in window — carry forward last known setpoint
...
def plot(s1, s2_base, s2_over, s3_base, s3_over,
    ...
    # One row per overridden zone + power subplot
    ...
    for ax_idx, z in enumerate(OVERRIDE_ZONES):
        ...
        for label, s2, s3, col in [
            ...
            ax.plot(t1 + t2 + s3_dts, v1 + v2 + s3_room,
                    color=col, linewidth=lw, linestyle=ls,
                    label=f"{label} — room temp")
            ...
            if label == "Override":
                ...
                v2s = s2_setp[:len(t2s)]
                ax.step(t1s + t2s + s3_dts_s, v1s + v2s + s3_setp,
                        where="post", color=col, linewidth=0.9,
                        linestyle=":", alpha=0.6, label="Override setpoint")

        for ax_ in (ax,):
            ax_.axvline(over_line, color="grey", linestyle="--", linewidth=0.9, alpha=0.6)
            ax_.axvline(end_line,  color="grey", linestyle=":",  linewidth=0.9, alpha=0.6)
            ax_.grid(axis="y", linestyle="--", alpha=0.3)
            ax_.grid(axis="x", which="minor", linestyle=":", alpha=0.2)
    ...
    for label, s2, s3, col in [
        ...
        ls = "--" if label == "Baseline" else "-"
        lw = 1.4  if label == "Baseline" else 1.8
        ax_power.plot(t1p + t2p + s3_dts_p, v1p + v2p + s3_pow,
                      color=col, linewidth=lw, linestyle=ls,
                      label=f"{label} — power")
...
def main():
    ...
    print(f"  Override:     {zones_str} → {OVERRIDE_SETPOINT}°C "
    ...
    for z in OVERRIDE_ZONES:
        pk  = peak_room_temp(s2_over, override_dt.to_pydatetime(), z)
        hit = setpoint_reached_at(s2_over, override_dt.to_pydatetime(), z)
        end_temp = temp_at(s2_over, override_end_dt.to_pydatetime(), z)

        print(f"  Zone {z} peak room temp: {pk:.1f}°C" if pk else f"  Zone {z} peak room temp: n/a")
        if hit:
            mins = (hit - override_dt.to_pydatetime()).total_seconds() / 60
            print(f"  Zone {z}: {OVERRIDE_SETPOINT}°C reached at {hit.strftime('%H:%M')} (+{mins:.0f} min)")
        else:
            print(f"  Zone {z}: {OVERRIDE_SETPOINT}°C not reached in override window")
        
        print(f"  Zone {z} temp at {int(OVERRIDE_END_HOUR):02d}:00: {end_temp:.1f}°C" if end_temp else
          f"  Zone {z} temp at {int(OVERRIDE_END_HOUR):02d}:00: n/a")
